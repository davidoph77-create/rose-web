  ...
  const analyserMessage = async () => {
    ...
    if (deleteHardRoute.matched) {
      ...
      if (forcedDeleteFoundation.handled && forcedDeleteFoundation.draft) {
        ...
        setRoseReponse(forcedDeleteResolveResult.text);
        ajouterJournal(
          `V10-044B4 : Calendar DELETE HARD ROUTE / resolved=${forcedDeleteResolveResult.resolved} / confidence=${forcedDeleteResolveResult.confidence} / eventId=${forcedDeleteResolveResult.event?.id ?? "none"} / DELETE=NOT-SENT`
        );
        parler(forcedDeleteResolveResult.text);
        setMessage("");

        console.log(
          `[Rose V10-044B4] DELETE RESOLVER RESULT / resolved=${forcedDeleteResolveResult.resolved} / confidence=${forcedDeleteResolveResult.confidence} / eventId=${forcedDeleteResolveResult.event?.id ?? "none"} / DELETE=NOT-SENT`
        );
        return;
      }
    ...
    if (calendarDeleteFoundation.handled && calendarDeleteFoundation.draft) {
      ...
      setRoseReponse(calendarDeleteResolveResult.text);
      ajouterJournal(
        `V10-044B : Calendar DELETE EVENT RESOLVER / read-only / resolved=${calendarDeleteResolveResult.resolved} / confidence=${calendarDeleteResolveResult.confidence} / eventId=${calendarDeleteResolveResult.event?.id ?? "none"} / DELETE=NOT-SENT`
      );
      parler(calendarDeleteResolveResult.text);
      setMessage("");
      console.log(
        `[Rose V10-044B] CALENDAR DELETE EVENT RESOLVER / resolved=${calendarDeleteResolveResult.resolved} / confidence=${calendarDeleteResolveResult.confidence} / eventId=${calendarDeleteResolveResult.event?.id ?? "none"} / DELETE=NOT-SENT`
      );
