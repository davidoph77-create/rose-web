...
import { prepareCalendarDeleteFoundation } from "./src/core/v10/calendar_delete_foundation";
import { resolveGoogleCalendarEventForDelete } from "./src/core/v10/calendar_delete_resolver";
import { classifyCalendarDeleteHardRoute } from "./src/core/v10/calendar_delete_hard_route";
import {
  createCalendarDeleteApprovalState,
  rememberPendingCalendarDelete,
  handleCalendarDeleteApproval,
...
export default function App() {
  ...
  // Rose V10-044C - secure DELETE approval state.
  // Local conversational confirmation only. Google Calendar DELETE HTTP stays disabled.
  const calendarDeleteApprovalRef = useRef(
    createCalendarDeleteApprovalState()
  );
  ...
  const analyserMessage = async () => {
    ...
    if (calendarDeleteApproval.handled) {
      if (calendarDeleteApproval.approved) {
    ...
    // Rose V10-044B4 - ABSOLUTE DELETE ROUTING GUARD.
    ...
    console.log(
      `[Rose V10-044B4] DELETE CLASSIFY / startsExplicitly=${deleteHardRoute.startsExplicitly} / verb=${deleteHardRoute.hasDeleteVerb} / calendarObject=${deleteHardRoute.hasCalendarObject} / matched=${deleteHardRoute.matched}`
    );

    if (deleteHardRoute.matched) {
      console.log(
        "[Rose V10-044B4] DELETE HARD ROUTE / matched=true / before-calendar-read"
      );

      const forcedDeleteFoundation =
        prepareCalendarDeleteFoundation(messageEnvoye);

      if (forcedDeleteFoundation.handled && forcedDeleteFoundation.draft) {
        ...
        const forcedDeleteResolveResult =
          await resolveGoogleCalendarEventForDelete(
            forcedDeleteFoundation.draft
          );

        let deleteResponseText = forcedDeleteResolveResult.text;

        if (forcedDeleteResolveResult.resolved && forcedDeleteResolveResult.event?.id) {
          calendarDeleteApprovalRef.current = rememberPendingCalendarDelete(
            calendarDeleteApprovalRef.current,
            forcedDeleteResolveResult.event
          );
          const deleteLabel = String(
            forcedDeleteResolveResult.event?.summary ??
            forcedDeleteResolveResult.event?.title ??
            "cet evenement"
          );
          deleteResponseText =
            `J'ai retrouve « ${deleteLabel} ». Veux-tu vraiment le supprimer de Google Calendar ? ` +
            `Reponds exactement « Oui je confirme » pour autoriser l'etape suivante, ou « Non » pour annuler. ` +
            `V10-044C n'envoie encore aucune suppression reelle.`;
        }

        setRoseReponse(deleteResponseText);
        ajouterJournal(
          `V10-044C : Calendar DELETE RESOLVED / resolved=${forcedDeleteResolveResult.resolved} / confidence=${forcedDeleteResolveResult.confidence} / eventId=${forcedDeleteResolveResult.event?.id ?? "none"} / approval-required / DELETE=NOT-SENT`
        );
        parler(deleteResponseText);
        setMessage("");

        console.log(
          `[Rose V10-044C] DELETE RESOLVER RESULT / resolved=${forcedDeleteResolveResult.resolved} / confidence=${forcedDeleteResolveResult.confidence} / eventId=${forcedDeleteResolveResult.event?.id ?? "none"} / approval-required / DELETE=NOT-SENT`
        );
        return;
      }

      const safeDeleteText =
        "J’ai reconnu une demande de suppression Google Calendar, mais je n’ai pas pu identifier précisément l’événement. Aucune suppression n’a été envoyée.";
      setRoseReponse(safeDeleteText);
      ajouterJournal(
        "V10-044B4 : Calendar DELETE HARD ROUTE / foundation-unhandled / DELETE=NOT-SENT / READ-FALLTHROUGH=BLOCKED"
      );
      parler(safeDeleteText);
      setMessage("");
      console.log(
        "[Rose V10-044B4] DELETE HARD ROUTE / foundation-unhandled / READ-FALLTHROUGH=BLOCKED"
      );
      return;
    }

    // Rose V10-044A - Calendar DELETE foundation.
    // SECURITY: intent detection + local draft only. NO Google Calendar DELETE is sent.
    const calendarDeleteFoundation = prepareCalendarDeleteFoundation(messageEnvoye);
    if (calendarDeleteFoundation.handled && calendarDeleteFoundation.draft) {
      const categorie = detecterCategorie(messageEnvoye);
      const importance = detecterImportance(messageEnvoye);
      ajouterMemoire(messageEnvoye, categorie, importance);

      const calendarDeleteResolveResult = await resolveGoogleCalendarEventForDelete(
        calendarDeleteFoundation.draft
      );
      let deleteResponseText = calendarDeleteResolveResult.text;
      if (calendarDeleteResolveResult.resolved && calendarDeleteResolveResult.event?.id) {
        calendarDeleteApprovalRef.current = rememberPendingCalendarDelete(
          calendarDeleteApprovalRef.current,
          calendarDeleteResolveResult.event
        );
        const deleteLabel = String(
          calendarDeleteResolveResult.event?.summary ??
          calendarDeleteResolveResult.event?.title ??
          "cet evenement"
        );
        deleteResponseText =
          `J'ai retrouve « ${deleteLabel} ». Veux-tu vraiment le supprimer de Google Calendar ? ` +
      ...
      setRoseReponse(deleteResponseText);
      ajouterJournal(
        `V10-044C : Calendar DELETE EVENT RESOLVER / resolved=${calendarDeleteResolveResult.resolved} / confidence=${calendarDeleteResolveResult.confidence} / eventId=${calendarDeleteResolveResult.event?.id ?? "none"} / approval-required / DELETE=NOT-SENT`
      );
      parler(deleteResponseText);
      setMessage("");
      console.log(
        `[Rose V10-044C] CALENDAR DELETE EVENT RESOLVER / resolved=${calendarDeleteResolveResult.resolved} / confidence=${calendarDeleteResolveResult.confidence} / eventId=${calendarDeleteResolveResult.event?.id ?? "none"} / approval-required / DELETE=NOT-SENT`
      );
      if (calendarDeleteResolveResult.error) console.log("[Rose V10-044B] Delete resolver error:", calendarDeleteResolveResult.error);
      return;
    }
